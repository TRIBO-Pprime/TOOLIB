...
contains
   ...
   type(sulu_env), intent(inout) :: sulu
      ...
      if (.not.allocated( sulu%perm_c )) allocate( sulu%perm_c(1:nn) )
      if (.not.allocated( sulu%perm_r )) allocate( sulu%perm_r(1:nn) )
      if (.not.allocated( sulu%etree  )) allocate( sulu%etree( 1:nn) )

      if (.not.allocated( sulu%RR )) allocate( sulu%RR(1:nn) )
      if (.not.allocated( sulu%CC )) allocate( sulu%CC(1:nn) )

      if (.not.allocated( sulu%ferr )) allocate( sulu%ferr(1:nb) )
      if (.not.allocated( sulu%berr )) allocate( sulu%berr(1:nb) )

      if (.not.allocated( sulu%rpg   )) allocate(   sulu%rpg(1:nb) )
      if (.not.allocated( sulu%rcond )) allocate( sulu%rcond(1:nb) )

      if (.not.allocated( sulu%x )) allocate( sulu%x(1:nn) )
